...
@Repository
public interface TssPostPersonDao extends BaseMapper<TssPostPerson> {
    ...
    @Select("SELECT post_name AS job_name,tpi_id_str FROM tss_post_person WHERE tep_id=#{tepId}")
    ...
    @Results({
            ...
            @Result(column = "kckd_id", property = "list", many = @Many(select = "com.byqj.dao.TssPostPersonDao.getPostNameAndPerson"))
    })
